  ...
  useEffect(() => {
    ...
    const handleChatBacklog = (data: { roomId: string; messages: any[]; isBacklog: boolean; syncType?: string }) => {
      ...
      for (const msg of data.messages) {
        const cmdTypes = ['cmd', 'cmdMe', 'cmdRoll', 'cmdGift', 'cmdShower', 'cmdGoal', 'cmdGo'];
        const isCommandMessage = cmdTypes.includes(msg.messageType) || cmdTypes.includes(msg.type);
        const isPresenceMessage = msg.messageType === 'presence' || msg.type === 'presence';
        ...
        const newMessage: Message = {
          ...
          isSystem: (msg.isSystem || msg.messageType === 'system' || msg.type === 'system') && !isPresenceMessage,
          isNotice: msg.messageType === 'notice',
          isCmd: isCommandMessage,
          isPresence: isPresenceMessage,
          ...
          type: msg.type,
          botType: msg.botType,
          hasTopMerchantBadge: msg.hasTopMerchantBadge,
          isTop1User: msg.isTop1User,
          hasTopLikeReward: msg.hasTopLikeReward,
          topLikeRewardExpiry: msg.topLikeRewardExpiry,
          userType: msg.userType || (msg.isModerator ? 'moderator' : (msg.isCreator ? 'creator' : 'normal')),
          bigEmoji: msg.bigEmoji,
          hasFlags: msg.hasFlags,
        };
        addMessageRef.current(data.roomId, newMessage);
      }
    };
    ...
    socket.off('chat:backlog', handleChatBacklog);
    ...
    socket.on('chat:backlog', handleChatBacklog);
    ...
    // DM Listener: Global auto-open tab
    const handleDmReceive = (data: any) => {
      devLog('📩 [Global DM] Received:', data.message);
      
      const otherUserId = data.fromUserId === currentUserIdRef.current ? data.toUserId : data.fromUserId;
      if (!otherUserId) return;

      const newMessage: Message = {
        id: data.id || `dm-${Date.now()}-${Math.random()}`,
        username: data.fromUsername || data.username,
        message: data.message,
        isOwnMessage: data.fromUserId === currentUserIdRef.current,
        timestamp: data.timestamp,
        messageType: 'dm',
        userType: data.fromRole || 'normal',
      };

      // addPrivateMessage handles tab creation and message insertion
      const addPrivateMessage = useRoomTabsStore.getState().addPrivateMessage;
      addPrivateMessage(otherUserId, newMessage);
    };
