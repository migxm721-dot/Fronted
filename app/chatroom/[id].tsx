...
export default function ChatRoomScreen() {
  ...
  useEffect(() => {
    ...
    const handlePmReceive = (data: any) => {
      ...
      const validPmTypes = ['pm', 'cmdGift', 'cmdMe', 'cmdRoll', 'cmd', 'cmdShower', 'gift'];
      if (!senderUsername || !message || (!validPmTypes.includes(data.messageType) && data.messageType !== 'cmdGift')) {
        return;
      }
      
      const { openRoom, addPrivateMessage, openRoomIds, currentUserId: storeUserId, markUnread } = useRoomTabsStore.getState();
        ...
        if (currentSocket) {
          if (needsForceReconnect || !currentSocket.connected) {
            ...
            if (activeRoomId && userId && username && 
                !activeRoomId.startsWith('private:') && !activeRoomId.startsWith('pm_')) {
              currentSocket.emit('room:silent_rejoin', {
                roomId: activeRoomId,
                userId: userId,
                username: username,
