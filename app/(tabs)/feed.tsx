...
export default function FeedScreen() {
  ...
  const pickVideo = async () => {
    ...
    const result = await ImagePicker.launchImageLibraryAsync({
      ...
      videoMaxDuration: 16, // Max 16 seconds
    });
    ...
    if (!result.canceled) {
      ...
      if (video.fileSize && video.fileSize > 50 * 1024 * 1024) {
        Alert.alert('Video Too Large', 'Video file size must be less than 50MB.');
        return;
      }
  ...
  const handleCreatePost = async () => {
    ...
    try {
      ...
      // Use AbortController for timeout (3 minutes for video)
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), selectedVideo ? 180000 : 60000);
      ...
      try {
        ...
        if (fetchError.name === 'AbortError') {
          Alert.alert('Timeout', 'Upload took too long. Please try a smaller video.');
        } else {
          throw fetchError;
        }
      }
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              const response = await fetch(API_ENDPOINTS.FEED.DELETE(String(postId)), {
                method: 'DELETE',
                headers: {
                  'Authorization': `Bearer ${token}`,
                  'x-device-id': deviceId || '',
  ...
  const renderPost = ({ item }: { item: Post }) => {
    ...
    <View style={[styles.postCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
      <View style={styles.postHeader}>
        ...
        {currentUserId && (item.user_id === currentUserId || item.userId === currentUserId) && (
          <TouchableOpacity 
            style={styles.deleteButton}
            onPress={() => handleDeletePost(item.id)}
          >
            <TrashIcon color="#FF5252" size={20} />
          </TouchableOpacity>
        )}
      </View>
