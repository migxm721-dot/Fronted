...
const EmojiIcon = ({ size = 20, color = '#999' }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24">
    <Path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z" fill="none" stroke={color} strokeWidth="2" />
...
export const ChatRoomInput = forwardRef<ChatRoomInputRef, ChatRoomInputProps>(({ 
  ...
  const handleSend = () => {
    ...
    if (lowerMsg.startsWith('/roll')) {
      ...
      setInputHeight(42);
      onSend(msgToSend);
      // Keyboard.dismiss(); // Keep keyboard open
    ...
    <Animated.View 
      style={[
        ...
        { 
          ...
          borderTopWidth: 1,
          borderTopColor: 'rgba(255, 255, 255, 0.1)', // Subtle white divider
        },
        animatedStyle,
      ]}
    >
...
const styles = StyleSheet.create({
  ...
  container: {
    ...
    alignItems: 'center',
    paddingHorizontal: 8,
    paddingVertical: 14, // Increased to 14 for more top/bottom spacing
    gap: 6,
  },
  iconButton: {
    padding: 6,
  ...
  inputContainer: {
    ...
    paddingVertical: 2, // Reduced to keep the overall bar size balanced
  },
  input: {
    fontSize: 14,
    minHeight: 36, // Slightly shorter input field to allow more container padding
    maxHeight: 120,
  },
  sendButton: {
    padding: 6,
