...
const ImageIcon = ({ size = 22, color = '#999' }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24">
    <Path
...
export const PrivateChatInput = forwardRef<PrivateChatInputRef, PrivateChatInputProps>(({ 
  ...
  const handleSend = () => {
    if (message.trim()) {
      ...
      setMessage('');
      setInputHeight(42);
      // Keyboard.dismiss() is not explicitly called here, but ensuring it stays open
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: '#0a5229' }]}>
      <View style={[styles.inputContainer, { backgroundColor: theme.card }]}>
        ...
        >
          <EmojiIcon size={22} color={emojiPickerVisible ? theme.primary : theme.secondary} />
        </TouchableOpacity>
        ...
        <TouchableOpacity 
          ...
          hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
        >
          <SendIcon size={22} color={message.trim() ? theme.primary : theme.secondary} />
        </TouchableOpacity>
      </View>
    </View>
  );
