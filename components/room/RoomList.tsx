...
import { ActiveRoomsModal } from './ActiveRoomsModal';
import { ChatIcon } from '@/components/ui/SvgIcons';
...
export function RoomList() {
  ...
  const [activeRoomsModalVisible, setActiveRoomsModalVisible] = useState(false);
  ...
  const activeRooms = useMemo(() => 
    openRoomIds
      .map(id => ({ id, name: openRoomsById[id]?.name || 'Chat' })),
  ...
  useEffect(() => {
      ...
      const updateRoom = (room: any) => {
        if (String(room.id) === String(data.roomId)) {
          return { 
            ...
            maxUsers: data.maxUsers || room.maxUsers || room.max_users || 50 
          };
        }
        return room;
      };
      
      setAllRooms(prev => prev.map(updateRoom));
      setRecentRooms(prev => prev.map(updateRoom));
      setOfficialRooms(prev => prev.map(updateRoom));
      setFavoriteRooms(prev => prev.map(updateRoom));
      setGameRooms(prev => prev.map(updateRoom));
    };
    ...
    socket.off('room:count:update', handleRoomsUpdateCount);
    ...
    socket.on('room:count:update', handleRoomsUpdateCount);
  ...
  const handleRoomPress = (roomId: string, roomName?: string) => {
    ...
    // Check if room is already open (could be a chatroom or a private chat)
    const store = useRoomTabsStore.getState();
    const isAlreadyOpen = store.openRoomIds.includes(roomId);

    if (isAlreadyOpen) {
      // If already open, just switch to it
      store.setActiveRoomById(roomId);
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <ActiveRoomsModal
        visible={activeRoomsModalVisible}
        onClose={() => setActiveRoomsModalVisible(false)}
      ...
      {activeRooms.length > 0 && (
        <TouchableOpacity 
          ...
          onPress={() => setActiveRoomsModalVisible(true)}
          activeOpacity={0.7}
        >
          <ChatIcon color="#fff" size={28} />
          <View style={styles.fabBadge}>
            <Text style={styles.fabBadgeText}>{activeRooms.length}</Text>
          </View>
        </TouchableOpacity>
      )}
