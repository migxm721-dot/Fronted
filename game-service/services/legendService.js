...
const generateResults = (roomId) => {
  ...
  // Chance to trigger a "Lucky Pattern" (25% chance)
  // This makes it feel dynamic and less predictable than pure random
  const luckyChance = Math.random();
  if (luckyChance < 0.25) {
    const luckyGroup = groupKeys[Math.floor(Math.random() * groupKeys.length)];
    // 30% chance for x6, 70% chance for x5
    const count = Math.random() < 0.3 ? 6 : 5;
    
    for (let i = 0; i < count; i++) results.push(luckyGroup);
    
    // Fill remaining slot if x5
    while (results.length < 6) {
      results.push(groupKeys[Math.floor(Math.random() * groupKeys.length)]);
    }
    
    // Shuffle the results so it's not always the first slots
  ...
  // Normal random for other 75% of games
  for (let i = 0; i < 6; i++) {
