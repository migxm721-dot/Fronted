...
class LowCard(
    ...
    init {
        ...
        val message =
            if (amountJoinPot > 0.0) "Play now: !start to enter. Cost: USD $amountJoinPot. For custom entry, !start <entry_amount> " else "Play LowCard: !start. Need $minPlayers players."
        sendMessage(message)
                ...
                try {
                    ...
                    if (gameState === BotData.BotStateEnum.GAME_JOINING) {
                        ...
                        if (getPlayers().size < minPlayers) {
                            sendMessage("Joining ends. Not enough players. Need $minPlayers.")
