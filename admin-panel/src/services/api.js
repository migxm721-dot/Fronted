// Use .env variable in development/production
...
export const adminApi = {
  ...
  deleteUser: (id) =>
    apiCall(`/api/auth/admin/users/${id}`, { method: 'DELETE' }),
  ...
  // App Config
  getAppConfig: () => apiCall('/api/admin/app-config'),
  updateAppConfig: (key, value) =>
    apiCall(`/api/admin/app-config/${encodeURIComponent(key)}`, {
      method: 'PUT',
      body: JSON.stringify({ value }),
    }),

  // Pending accounts
  getPendingAccounts: (page = 1) => apiCall(`/api/admin/pending-accounts?page=${page}`),
  activateAccount: (id) =>
    apiCall(`/api/admin/users/${id}/activate`, { method: 'PATCH' }),
  bulkActivateAccounts: (userIds) =>
    apiCall('/api/admin/users/activate-bulk', {
      method: 'POST',
